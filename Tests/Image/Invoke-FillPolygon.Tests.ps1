...
Describe "基本処理" {
    It "描画" {
        try {
            ...
            Invoke-FillPolygon -Path $temp_file.FullName -Points @(
                @{x=20;y=20};
                @{x=80;y=30};
                @{x=30;y=40};
                @{x=70;y=50};
            ...
            Copy-Item -Path $temp_file.FullName -Destination "$PSScriptRoot/Outputs/FillPolygon_$(++$script:count;$script:count).bmp";
        } finally {
            $temp_file.Delete();
        }
    }
...
Describe "例外" {
    It "Pathが存在しない" {
        try {
            $path = "not_found";
            Invoke-FillPolygon -Path $path -Points @(
                @{x=20;y=20};
                @{x=80;y=30};
                @{x=30;y=40};
                @{x=70;y=50};
