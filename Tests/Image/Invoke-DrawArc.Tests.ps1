...
Describe "基本処理" {
    It "描画" {
        try {
            ...
            Invoke-DrawArc -Path $temp_file.FullName -Left 0 -Top 0 -Width 100 -Height 100 -StartAngle 0 -SweepAngle 180 -R 0xFF -G 0xFF -B 0xFF -A 0x7F;
    ...
    It "パイプ入力" {
        try {
            ...
            New-Image -Path $temp_file.FullName -Width 100 -Height 100 -PassThru `
                | Invoke-DrawArc -Left 0 -Top 0 -Width 100 -Height 100 -StartAngle 0 -SweepAngle 180 -R 0xFF -G 0xFF -B 0xFF -A 0x7F;
            Copy-Item -Path $temp_file.FullName -Destination "$PSScriptRoot/Outputs/DrawArc_$(++$script:count;$script:count).bmp";
        } finally {
            $temp_file.Delete();
        }
