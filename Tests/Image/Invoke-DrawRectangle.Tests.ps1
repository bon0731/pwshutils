...
Describe "基本処理" {
    It "描画" {
        try {
            ...
            Invoke-DrawRectangle -Path $temp_file.FullName -Left 25 -Top 15 -Width 50 -Height 70 -R 0xFF -G 0xFF -B 0xFF -A 0x7F;
    ...
    It "パイプ入力" {
        try {
            ...
            New-Image -Path $temp_file.FullName -Width 100 -Height 100 -PassThru `
                | Invoke-DrawRectangle -Left 25 -Top 15 -Width 50 -Height 70 -R 0xFF -G 0xFF -B 0xFF -A 0x7F;
            Copy-Item -Path $temp_file.FullName -Destination "$PSScriptRoot/Outputs/DrawRectangle_$(++$script:count;$script:count).bmp";
        } finally {
            $temp_file.Delete();
        }
...
Describe "例外" {
    It "Pathが存在しない" {
        try {
            $path = "not_found";
            Invoke-DrawRectangle -Path $path -Left 25 -Top 15 -Width 50 -Height 70 -R 0xFF -G 0xFF -B 0xFF -A 0x7F;
            throw "";
        } catch {
